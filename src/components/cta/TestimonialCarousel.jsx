...
export default function TestimonialCarousel() {
  ...
  return (
    <div className="relative w-full h-full max-h-[70svh] flex items-center justify-center overflow-hidden">
      
      <div className="absolute inset-0 min-h-screen h-full bg-gradient-to-b from-[#FCF9EA] to-[#97A87A] to-10%" />
      <Marquee pauseOnHover={true} vertical className="max-h-[70svh] font-semibold text-lg [--duration:60s]" repeat={3}>
        {reviews1.map((review, index) => (
          <SocialCard key={index} body={review.text} name={review.author} img={review.img} />
        ))}
      </Marquee>
      <Marquee pauseOnHover={true} vertical className="max-h-[70svh] font-semibold text-lg [--duration:60s]" repeat={3} reverse>
